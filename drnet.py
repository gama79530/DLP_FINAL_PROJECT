...
class Net(nn.Module):
    ...
    def __init__(self, classes, embed_dim, resnet, pretrained_model=None,
        ...
        assert(isinstance(classes, dict)
               ), f"num_labels should be dict, got {type(classes)}"
        ...
        for n_labels in classes.values():
            ...
            if use_torch_up:
                ...
                #                         bias=False)
                up = nn.Linear(n_labels*16, n_labels)
                # fill_up_weights(up)
        ...
        m.bias.data.zero_()
        self.en_map = m
        self.en_up = nn.ConvTranspose2d(self.embed_dim, self.embed_dim, 16, stride=8,
                                        padding=4, output_padding=0, groups=self.embed_dim, bias=False)
