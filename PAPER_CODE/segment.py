...
class load_data():
    ...
    def __init__(self, args):
        ...
        self.d_path = {entry.name: basedir +
                       entry.path for entry in self.metadata}
...
def train(args, get_dataset, model, enc=False):
    ...
    dataset_train = {dname: get_dataset(
        dname, 'train', args.num_samples) for dname in datasets}
    ...
    if entropy:
        ...
        n_unlabeled = np.max([len(dataset_unlabeled[dname])
    ...
    for key in datasets:
        print("{}: Unlabeled images {}, Training on {} images, Validation on {} images".format(
            key, len(dataset_unlabeled[key]), len(dataset_train[key]), len(dataset_val[key])))
            ...
            for itr in range(n_iters):
                ...
                for d in datasets:
                    ...
                    loss_s = loss_criterion[d](
                        dec_outputs[d], targets_l.squeeze(1))
                    loss_s.backward()
    ...
    if args.state:
        ...
        def load_my_state_dict(model, state_dict):
            ...
            state_dict = {k.partition(
                'module.')[2]: v for k, v in state_dict.items()}
